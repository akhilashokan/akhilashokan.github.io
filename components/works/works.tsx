                ...
                <div className={style.works_container} id="__id">
                    {works.map((work: Work, index) => {
                        ...
                        if (work.link.match(regxUrl) !== null) {
                            link = <a href={work.link} title={"Link to " + work.title} className={style.link} target={"_blank"}>
                        ...
                        return (
                            <div tabIndex={index} title={work.title} className={style.work} key={index} style={{ backgroundImage: `url(${thumb})` }} >
                                <h3>{work.title}</h3>
                                <p>{work.buildWith}</p>
                                {link}
                            </div>
